...
```
### Line A
Forms a matrix with x rows param columns where each intersect and are evaluted at 1fun.

### Line B
Forms a logic vector of whether y is max of y then outputs the largest indices in which a true is outputted

### Likelihood & R Code
$$L(p) = (y_1,y_2,y_3, y_4, y_5, y_6,y_7,y_8)= p(y_1)p(y_2)p(y_3)p(y_4)p(y_5)p(y_6)p(y_7)p(y_8)$$


$$= \binom{20}{y_1} p^{y_1} (1 - p)^{20 - y_1} \binom{20}{y_2} p^{y_2} (1 - p)^{20 - y_2} \dots \binom{20}{y_8} p^{y_8} (1 - p)^{20 - y_8}
...
$$
L(\lambda) = \prod_{i=1}^5 \frac{e^{-\lambda} \lambda^{y_i}}{y_i!}
$$

$$
l(\lambda) = \log(e^{-5\lambda}) + \log(\lambda^{\sum_{i=1}^5 y_i}) - \sum_{i=1}^5 \log(y_i!)
$$

$$
l(\lambda) = -5\lambda + \sum_{i=1}^5 y_i \log(\lambda) - \sum_{i=1}^5 \log(y_i!)
$$
...
## Task 4
$$L(p)=\binom{6}{2} p^{2}(1 - p)^{4}*\binom{10}{4} p^{4}(1 - p)^{6}$$
$$L(p)=log|\binom{6}{2} p^{2}(1 - p)^{4}*\binom{10}{4} p^{4}(1 - p)^{6}|$$

### The maximum likelihood 
The maximum likelihood is 0.375
```{r}
...
## Task 5
 $$
L(\theta_1, \theta_2) = \binom{n}{y_1} \theta_1^{y_1} (1 - \theta_1)^{n - y_1} \frac{e^{-\theta_2} \theta_2^{y_2}}{y_2!}
$$

The log-likelihood function \( l(\theta_1, \theta_2) \) is:

$$
l(\theta_1, \theta_2) = \log \left( \binom{n}{y_1} \theta_1^{y_1} (1 - \theta_1)^{n - y_1} \frac{e^{-\theta_2} \theta_2^{y_2}}{y_2!} \right)
$$

Expanding this further:

$$
= \log \left( \binom{n}{y_1} \right) + y_1 \log(\theta_1) + (n - y_1) \log(1 - \theta_1) - \theta_2 + y_2 \log(\theta_2) - \log(y_2!)
$$

Rewriting in terms of factorials:

$$
= \log \left( \frac{n!}{y_1! (n - y_1)!} \right) + y_1 \log(\theta_1) + (n - y_1) \log(1 - \theta_1) - \theta_2 + y_2 \log(\theta_2) - \log(y_2!)
$$

Further simplification:

$$
= \log(n!) - \log(y_1!) - \log((n - y_1)!) + y_1 \log(\theta_1) + (n - y_1) \log(1 - \theta_1) - \theta_2 + y_2 \log(\theta_2) - \log(y_2!)$$

### mylikg2() for graphical maximum likelihood
...
for(i in 1:12) {
    w <- mymlbeta(
        ...
        beta = seq(0.1, 20, length = 100), lwd=2,labcex=1,
        col = "steelblue"
    )
    points(a, b, col = "Red", pch = 19) # Adjust placement if necessary
