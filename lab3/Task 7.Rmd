...
renderPlot({
 ...
 g = ggplot(spruce.df, aes(x = BHDiameter, y = Height)) +  geom_smooth(method="lm")

 gh = g + geom_point(aes(col = Height)) 
 
 gr = g + geom_point(aes(col = RATIO)) 
 
 gsh =  g + geom_point(aes(size = BHDiameter, col = Height))
 
 gsr = g + geom_point(aes(size = BHDiameter, col = Height))
